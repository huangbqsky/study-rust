...
impl<T: Copy + Debug> LVec<T> {
    ...
    fn insert(&mut self, mut index: usize, val: T) {
        if index >= self.size {
            index = self.size;
        }

        let mut node = Node::new(val);
        if self.is_empty() { // LVec 为空，直接插入
            self.head = Some(Box::new(node));
    ...
    // 打印LVec，当然也可以实现ToString特性并用println打印
    fn print_lvec(&self) {
